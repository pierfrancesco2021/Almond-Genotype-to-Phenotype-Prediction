...
for (j in 1:15) {
  ...
  for (i in 1:10) {
    ...
    # Fit rrBLUP model
    rrblup_model <- mixed.solve(y = train_pheno, Z = train_geno, K = NULL, method = "REML")
    
    # Predict phenotypes for test set
    rrblup_predictions <- test_geno %*% rrblup_model$u + rep(rrblup_model$beta, nrow(test_geno))
    
    # Accumulate true and predicted values
    all_test_pheno <- c(all_test_pheno, test_pheno)
    all_predictions <- c(all_predictions, rrblup_predictions)
  }
...
#write.csv(coefficients_of_determination, file = "r2_rrblup.csv")
#write.csv(pearson_correlations, file = "corr_rrblup.csv")
#write.csv(rmses, file = "rmse_rrblup.csv")